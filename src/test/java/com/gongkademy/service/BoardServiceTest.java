...
//        em.persist(course);
//        em.persist(lecture);
//        Question question = Question.builder().title("질문0").body("질문내용0").member(member).course(course).lecture(lecture).build();
//        Worry worry = Worry.builder().title("고민0").body("고민내용0").member(member).build();
//        em.persist(question);
//        em.persist(worry);
//
//        for(int i=0;i<100;i++){
//            Question question1 = Question.builder().title("질문1").body("질문내용1").member(member).course(course).lecture(lecture).build();
...
//    void 게시글_상세_조회(){
//        //given
//        Member member = Member.builder().nickname("유저1").name("user1").email("aaa@naver.com").provider(Provider.KAKAO).providerId("111").build();
//        Course course = Course.builder().title("재료역학").thumbnail("aaa").build();
//        Lecture lecture = Lecture.builder().title("재료역학이란").lectureOrder(1).url("ys5niu4Sabg&list=PLwzYFiJ0Ed6kGyX0M_IW1LGiLO_Ggh4Jy")
//                                 .runtime(15 * 60).course(course).build();
//
...
//        em.persist(lecture);
//
//        WriteBoardRequest questionRequest = WriteBoardRequest.builder().title("질문1").body("질문내용1").courseId(course.getId()).lectureId(lecture.getId()).build();
...
//        BoardDetailResponse findQuestion = boardService.findBoardDetail(member.getId(),questionId);
//
//
//        //then
...
//        em.persist(lecture2);
//
//        WriteBoardRequest questionRequest = WriteBoardRequest.builder().title("질문1").body("질문내용1").courseId(course.getId()).lectureId(lecture1.getId()).build();
//        //when
//        Long questionId = boardService.write(member.getId(),questionRequest , QUESTION);
//
//        boardService.edit(member.getId(), questionId, EditBoardRequest.builder().body("고민내용2").courseId(course.getId()).lectureId(lecture2.getId()).build(), QUESTION);
//        Question findQuestion = em.find(Question.class,questionId);
//
//        //then
...
//    void 게시글_삭제(){
//            //given
//            Member member1 = Member.builder().name("유저1").provider(Provider.KAKAO).providerId("aaa").nickname("유저1").email("aaa@naver.com").build();
//            Member member2 = Member.builder().name("유저1").provider(Provider.KAKAO).providerId("aaa").nickname("유저2").email("bbb@naver.com").build();
//            Course course = Course.builder().title("재료역학").thumbnail("aaa").build();
//            Lecture lecture = Lecture.builder().title("재료역학이란").lectureOrder(1).url("ys5niu4Sabg&list=PLwzYFiJ0Ed6kGyX0M_IW1LGiLO_Ggh4Jy")
//                                     .runtime(15 * 60).course(course).build();
//
